...
public class AlgorithmUtility {
	...
	public static boolean checkAnagram(String first, String second) {
		/**
		 * Make hash table for both the string. String have character from a to
		 * z. And finally check that the count at every position is same, if yes
		 * return Strings are anagram else return Strings are not anagram.
		 */
	...
	public static boolean binarySearchForString(List<String> arr, String key, int len) {
		...
		while (low <= high) {
			...
			if (tmpstr.compareTo(key) > 0)
				high = mid - 1;
			else if (tmpstr.compareTo(key) < 0)
				low = mid + 1;
			else
				return true;
		}
